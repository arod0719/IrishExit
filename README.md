...
- **Per-Meeting Opt-In (OFF by Default)**:
  ...
  - Arm IrishExit with a single click using the in-call floating pill on the bottom-left of Google Meet or via the toolbar popup.
  - *(Optional)* Check **"Auto-Turn ON for Every New Meeting"** if you prefer it active automatically on every call.
- **Adaptive Scaling (5–10 Person Syncs up to 100+ Person All-Hands)**:
  ...
  - **Minimum Room Floor** (default `≤ 2` participants left):
...
- **Exit Presets**:
  ...
  - **Slow Exit**: `-40%` drop / `≤ 2` floor (extra leeway for large presentations).
